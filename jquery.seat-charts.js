...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
	  ...
	  var caret = 0; // this caret start rows for new block of tickets like balcony with separate row numbers
	  $.each(settings.map, function(row, characters) {
		...
		if(emptyMap[row]){
		  ...
		  // if empty row has a # hash sign then we tink its new counter for row
		  if(emptyMap[row].match(/[#]/)){
			caret = row;
		  }
		  ...
		  if(fm){
			...
			$.each(fm.match(/[+*#]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) {
			  ...
			  $f.append((character != '+' && character != '#') ?
				//if the character is not a plus (empty space)
				(function(naming) {
		...
		if (settings.naming.left) {
		  $row.append(
			$('<div></div>')
			  ...
			  .text(settings.naming.rows[row - caret])
		  );
		}
